...
module.exports = class EPTranslation {
	...
	loadLanguageFile(languageFile) {
		...
		const languageFullname = path.resolve(process.cwd(), `${this.languageFolder}/${languageFile}`);

		this.translationMap[languageKey] = require(languageFullname);
	}
	...
	f(translateKey, args) {
		...
		for (let i = 0; i < argKeys.length; i += 1) {
			translated = translated.replace(new RegExp(`{{${argKeys[i]}}}`, 'g'), args[argKeys[i]]);
		}
